...
- Always: Yahoo Finance (price + RSI) — free, unlimited
...
ANALYST = {
    "MSFT":  {"buy": 59, "hold": 3,  "sell": 0,  "target": 559.96},
    "AAPL":  {"buy": 33, "hold": 15, "sell": 3,  "target": 316.07},
    "NVDA":  {"buy": 66, "hold": 4,  "sell": 1,  "target": 309.01},
    "AMZN":  {"buy": 68, "hold": 3,  "sell": 0,  "target": 317.41},
    "GOOGL": {"buy": 62, "hold": 10, "sell": 0,  "target": 433.46},
    "META":  {"buy": 64, "hold": 6,  "sell": 0,  "target": 822.34},
    "TSLA":  {"buy": 23, "hold": 22, "sell": 8,  "target": 395.20},
}
...
def load_existing():
    try:
        ...
        return {"lastFmpUpdate": None, "lastYahooUpdate": None, "stocks": []}
...
def main():
    ...
    print(f"Run at {now.isoformat()} | FMP update: {do_fmp}")

    # ── Yahoo Finance (always) — parallel ──────────────────────────────────────
    yahoo_results = {}
    ...
    if do_fmp:
        ...
        with ThreadPoolExecutor(max_workers=10) as ex:
            futures = {ex.submit(fn_map[kind], ticker): (kind, ticker) for kind, ticker in fmp_tasks}
            for future in as_completed(futures):
                r = future.result()
    ...
    print(f"Done. Wrote {len(stocks)} stocks. FMP updated: {do_fmp}")
